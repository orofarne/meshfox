...
pub fn launch() -> Result<Browser, String> {
    ...
    let launch_options =
        LaunchOptions::default_builder().path(path).build().map_err(|e| format!("chrome launch options: {e}"))?;
    Browser::new(launch_options).map_err(|e| format!("failed to launch a Chrome/Chromium browser: {e}"))
}

/// Navigates a fresh tab to `html_path` (a local file) and prints it to PDF
/// bytes with `options`.
pub fn print_file(browser: &Browser, html_path: &Path, options: PrintToPdfOptions) -> Result<Vec<u8>, String> {
    let tab = browser.new_tab().map_err(|e| format!("failed to open a browser tab: {e}"))?;
    navigate(&tab, html_path)?;
    tab.print_to_pdf(Some(options)).map_err(|e| format!("failed to print {} to PDF: {e}", html_path.display()))
}
...
fn navigate(tab: &Tab, html_path: &Path) -> Result<(), String> {
    ...
    tab.navigate_to(&url).map_err(|e| format!("failed to navigate to {}: {e}", html_path.display()))?;
...
fn wait_for_load(tab: &Tab) -> Result<(), String> {
    ...
    loop {
        let ready = tab
            .evaluate("document.readyState === 'complete' && location.href !== 'about:blank'", false)
            .ok()
...
fn wait_for_diagram_size(tab: &Tab) -> Result<(f64, f64), String> {
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            return Err("timed out waiting for the canvas page's own layout script to finish".to_string());
        }
        std::thread::sleep(Duration::from_millis(50));
    }
}
...
fn read_number(tab: &Tab, expression: &str) -> Option<f64> {
    tab.evaluate(expression, false).ok().and_then(|obj| obj.value).and_then(|v| v.as_f64())
}
...
pub fn document_options() -> PrintToPdfOptions {
    PrintToPdfOptions { prefer_css_page_size: Some(true), print_background: Some(true), ..Default::default() }
}
